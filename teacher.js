...
const sb=createClient(SUPABASE_URL,SUPABASE_PUBLISHABLE_KEY),$=id=>document.getElementById(id);let vocab=[],examples=[],cache=[];
const esc=v=>String(v??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#039;'}[c]));
function shuffle(a){a=[...a];for(let i=a.length-1;i;i--){const j=Math.floor(Math.random()*(i+1));[a[i],a[j]]=[a[j],a[i]]}return a}
function grades(){return[{min:+$('g1p').value,grade:'1+'},{min:+$('g1').value,grade:'1'},{min:+$('g2p').value,grade:'2+'},{min:+$('g2').value,grade:'2'},{min:+$('g3').value,grade:'3'},{min:+$('g4').value,grade:'4'},{min:0,grade:'5'}].sort((a,b)=>b.min-a.min)}
async function init(){const [d,x]=await Promise.all([fetch('daten/band4_unite1.json').then(r=>r.json()),fetch('daten/band4_unite1_examples.json').then(r=>r.json())]);vocab=d.entries;examples=x.examples||[];const secs=[...new Set(vocab.map(e=>e.section))];$('section').innerHTML='<option value="all">Alle Bereiche</option>'+secs.map(x=>`<option>${esc(x)}</option>`).join('');const {data:{session}}=await sb.auth.getSession();toggle(!!session);if(session)refresh()}
function toggle(on){$('loginBox').classList.toggle('hidden',on);$('teacherBox').classList.toggle('hidden',!on)}
